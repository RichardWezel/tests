...
class TestCreateQuestion(APITestCase):
   ...
   def test_author_is_bigNumber_not_fk(self):
      # ARRANGE
      url = reverse('question-list')
      data = {
          'title': 'Wie funktioniert DRF?',
   ...
   def test_create_question_without_title(self):
      # ARRANGE
      url = reverse('question-list')
      data = {
          'content': 'Fehlender Titel!',
          'category': 'frontend',
          'author': self.user.id
      }
      # ACT
...
class TestUpdateQuestion(APITestCase):
...
# class TestDeleteQuestion(APITestCase):
